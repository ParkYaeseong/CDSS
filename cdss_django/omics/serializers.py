...
class OmicsRequestListSerializer(serializers.ModelSerializer):
    """
    환자별 오믹스 분석 요청 목록을 위한 Serializer
    """
    # patient 필드는 ID만 필요하므로 그대로 두고, requester는 제외합니다.
    # request_timestamp를 analysis_date라는 이름으로 프론트엔드에 전달할 수 있습니다.
    analysis_date = serializers.DateTimeField(source='request_timestamp', format='%Y-%m-%d %H:%M')

    class Meta:
        model = OmicsRequest
        # [수정] id, analysis_date, cancer_type, status 필드를 목록에 포함합니다.
        fields = ['id', 'analysis_date', 'cancer_type', 'status']
